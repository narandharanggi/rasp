...
public class AlgoritmaGenetika {
    ...
    public AlgoritmaGenetika(int ukuranPopulasi, int jumlahGenerasi, double probMutasi, double probCross, int elitism, int batas, double target[],List<BahanPakan> bp){
...
//        System.out.println("fitness "+atribut[1]);
        ...
        individu.setHarga((int)roundFormat(harga));
        individu.setFitness(atribut[1]);
    }
    
    public double harga(Individu individu, List<BahanPakan> bp){
        double total=0;
        for (int i = 0; i < 4; i++) {
